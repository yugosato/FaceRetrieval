...
#pragma pack(2)
  ...
  public:
    String &operator=(const string &s) {
      ...
      strcpy((char*)c_str(), s.c_str());
      return *this;
    }
    size_t size() { return PARENT::size() - 1; }
    const char* c_str() {return (char*)&(this->at(0));}
#else
#ifdef NGT_COMPACT_VECTOR
  class String : public CompactString {
    typedef CompactString PARENT;
#else
  ...
  public:
...
#endif
    void serialize(ofstream &os, ObjectSpace *objspace = 0) {
      if (PARENT::size() > 0xFFFF) {
	cerr << "Too long identifier!" << endl;
	assert((PARENT::size()) <= 0xFFFF);
      }
      NGT::Serializer::write(os, (unsigned short)size());    
      os.write(c_str(), PARENT::size());
    }
	...
	assert((PARENT::size()) <= 0xFFFF);
      ...
      NGT::Serializer::writeAsText(os, (unsigned short)size());    
      os << " ";
      os.write(c_str(), size());
    }
    ...
    void deserializeAsText(ifstream &is, ObjectSpace *objectspace = 0) {
      PARENT::clear();
      unsigned short sz;
      NGT::Serializer::readAsText(is, sz);
      if (sz != 0) {
	string buf;
	is >> buf;
	*this = buf;
	cerr << "buf=" << buf.size() << "[" << buf << "]" << endl;
	assert(buf.size() == sz);
      } else {
	*this = "";
      }
